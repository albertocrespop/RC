...
public class NFController {
	 ...
	 */
	private static final byte BG_SERVER = 2;
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_LOGIN:
			if (currentState != LOGGED_OUT) {
				...
				System.out.println("* You cannot login because you are not logged out from the directory.");
			}
			break;
		case NFCommands.COM_LOGOUT:
			if(currentState != LOGGED_IN) {
				commandAllowed = false;
				System.out.println("* You cannot logout because you are not logged into the directory.");
			}
			break;
		case NFCommands.COM_FGSERVE:
			if(currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.out.println("* You can't start a server until you log into the directory.");
			}
			else if(currentState == BG_SERVER) {
				commandAllowed = false;
				System.out.println("* You can't start FG Server while you have a BG Server started.");
			}
			break;
		case NFCommands.COM_BGSERVE: {
			if(currentState == LOGGED_OUT) {
		...
		case NFCommands.COM_USERLIST: {
			if(currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.out.println("* You need to be logged into a directory to use this command.");
			}
			break;
		}
		case NFCommands.COM_DOWNLOADFROM: {
			if(currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.out.println("* You need to be logged into a directory to download files from a server.");
			}
			break;
		}
		case NFCommands.COM_STOP_SERVER:{
			if(currentState != BG_SERVER) {
				commandAllowed = false;
				System.out.println("* You need to start a server to stop it.");
			}
			break;
		}
		default:
